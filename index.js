  ...
  try {
    ...
    if (text === "➕ أنشئ حساب Gmail جديد") {
      ...
      bot.sendMessage(chatId,
        ...
        `👤 الاسم: <code>${escapeHtml(account.firstName)} ${escapeHtml(account.lastName)}</code>\n` +
        ...
        `⚠️ توجه إلى إعدادات جوجل وفعّل (التحقق بخطوتين)، ثم استخرج <b>الأكواد الاحتياطية (Backup Codes)</b> واضغط على الزر بالأسفل.`,
    ...
    if (text === "✅ تم التفعيل والإنشاء") {
      ...
      await user.save();
      bot.sendMessage(chatId, "🔐 <b>أرسل الآن (أكواد النسخ الاحتياطي الـ 8) الخاصة بحساب الـ Gmail لحفظ أمانه:</b>", { parse_mode: "HTML", reply_markup: { keyboard: [["❌ إلغاء إنشاء الحساب"]], resize_keyboard: true } }).catch(() => {});
      return;
    }

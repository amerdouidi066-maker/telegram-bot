...
const B = {
  ...
  ADMIN_CHANGE_PRICE:    "💲 تغيير سعر الإنشاء",        // ✅ جديد
...
// ✅ جديد: قائمة بديلة عند فشل التحقق مع خيار إعادة المحاولة
...
const ADMIN_MENU = {
  reply_markup: {
    keyboard: [
      ...
      [B.ADMIN_CHANGE_PRICE,    B.ADMIN_CLEAR_STOCK],   // ✅ جديد
      [B.ADMIN_EXIT],
    ],
    resize_keyboard: true,
  },
...
// ✅ جديد: تحميل السعر من DB عند التشغيل (يُبقي القيمة في الذاكرة للأداء)
async function loadDynamicPrice() {
  try {
    ...
    if (doc && typeof doc.value === "number" && doc.value > 0) {
      ...
      // أول تشغيل: احفظ السعر الافتراضي
...
// ═══════════════════════════════════════════════
// 🌐 التحقق من وجود Gmail على سيرفرات جوجل
// ═══════════════════════════════════════════════
/**
 * ✅ منطق الدالة:
 * errorMessage فارغ  = لا خطأ = الاسم متاح     = الحساب غير منشأ بعد  → valid: false
 * errorMessage موجود = الاسم محجوز              = الحساب منشأ فعلاً    → valid: true
 */
function verifyEmail(email) {
  return new Promise((resolve) => {
    const emailRegex = /^[a-z0-9]([a-z0-9._]{4,28}[a-z0-9])@gmail\.com$/i;
    if (!emailRegex.test(email)) {
      return resolve({ valid: false, reason: "صيغة البريد الإلكتروني غير صالحة." });
    }

    const body = JSON.stringify({ username: email.split("@")[0], locale: "en_US" });
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          const usernameError = json?.inputErrors?.username?.errorMessage;
          if (usernameError === "") {
            // errorMessage فارغ = لا خطأ = الاسم متاح = الحساب غير موجود
            return resolve({ valid: false, reason: "لم يتم إنشاء هذا البريد فعلاً على جوجل." });
          }
          // errorMessage موجود = الاسم محجوز = الحساب موجود ✅
          return resolve({ valid: true, reason: "الحساب منشأ وموجود فعلاً." });
        } catch {
    ...
    req.on("timeout", () => { req.destroy(); resolve({ valid: true, reason: "مهلة الاتصال، مرور احتياطي." }); });
    req.on("error",   () => resolve({ valid: true, reason: "خطأ في الاتصال، مرور احتياطي." }));
    req.write(body);
    req.end();
  });
}
...
async function handleConfirmation(user, msg, chatId, text) {
  ...
  const { valid, reason } = await verifyEmail(account.email);
  ...
  await bot.sendMessage(chatId,
    `✅ <b>تم التحقق! الحساب موجود فعلاً على جوجل.</b>\n\n` +
    `⚠️ <b>الخطوات التالية الإلزامية:</b>\n\n` +
    ...
    `💡 اضغط <b>❓ كيفية التفعيل</b> للمساعدة التفصيلية.\n` +
...
mongoose.connect(MONGODB_URI)
  .then(async () => {
    ...
    await loadDynamicPrice(); // ✅ جديد: تحميل السعر المحفوظ من DB
  })

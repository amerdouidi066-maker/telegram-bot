...
async function cleanupStaleSessions() {
  ...
  if (stuckUsers.length > 0) console.log(`🧹 تم تنظيف ${stuckUsers.length} جلسة معلقة`);
}

async function verifyEmail(email) {
  try {
    const emailRegex = /^[a-zA-Z0-9._%+-]+@gmail\.com$/;
    if (!emailRegex.test(email)) return { valid: false, reason: "صيغة الإيميل غير صحيحة" };
    const mxRecords = await dns.resolveMx("gmail.com");
    if (!mxRecords || mxRecords.length === 0) return { valid: false, reason: "تعذر التحقق من النطاق" };
...
async function autoReviewTasks() {
  ...
  const expiredTasks = await Task.find({ status: "pending", submittedAt: { $lte: seventyTwoHoursAgo }, autoChecked: false });
  for (const task of expiredTasks) {
    const result = await verifyEmail(task.accountEmail);
    task.autoChecked = true; task.checkResult = result.reason;
    if (result.valid) {
      ...
      if (user) {
        ...
        bot.sendMessage(task.userId, `✅ *تمت الموافقة على حسابك تلقائياً!*\n\n📧 \`${task.accountEmail}\`\n💵 تم إضافة *$${task.amount} USDT* لرصيدك!\n💰 رصيدك الحالي: *$${fmt(user.balance)} USDT*`, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
      }
      bot.sendMessage(ADMIN_ID, `✅ *موافقة تلقائية*\n\n📧 \`${task.accountEmail}\`\n👤 \`${task.userId}\`\n💵 $${task.amount} USDT`, { parse_mode: "Markdown" }).catch(() => {});
    } else {
      ...
      const user = await User.findOne({ telegramId: task.userId });
      if (user) bot.sendMessage(task.userId, `❌ *تم رفض حسابك تلقائياً*\n\n📧 \`${task.accountEmail}\`\nالسبب: ${result.reason}\n\nيمكنك المحاولة مرة أخرى.`, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
      bot.sendMessage(ADMIN_ID, `❌ *رفض تلقائي*\n\n📧 \`${task.accountEmail}\`\n👤 \`${task.userId}\`\nالسبب: ${result.reason}`, { parse_mode: "Markdown" }).catch(() => {});
    }
  }
}
...
bot.on("message", async (msg) => {
  ...
  try {
    ...
    if (text === "✅ تم") {
      if (user.state !== "awaiting_confirmation") { bot.sendMessage(chatId, "❌ لا يوجد حساب نشط.", MAIN_MENU); return; }
      const accountId = user.stateMeta?.accountId;
      const account = await Account.findById(accountId);
      if (!account) { user.state = null; user.stateMeta = null; await user.save(); bot.sendMessage(chatId, "❌ حدث خطأ. حاول مرة أخرى.", MAIN_MENU); return; }
      bot.sendMessage(chatId, `🔍 *جاري التحقق...*`, { parse_mode: "Markdown" });
      ...
      if (!verification.valid) {
        ...
        bot.sendMessage(chatId, `❌ *تم رفض الحساب تلقائياً*\n\nالسبب: ${verification.reason}\n\nيمكنك المحاولة مرة أخرى.`, { parse_mode: "Markdown", ...MAIN_MENU }); return;
      }
      await Account.findByIdAndUpdate(accountId, { recoveryEmail: RECOVERY_EMAIL });
      const task = await Task.create({ userId: user.telegramId, amount: 0.17, accountEmail: account.email, accountId: account._id, submittedAt: new Date() });
      user.state = null; user.stateMeta = null; await user.save();
      ...
      bot.sendMessage(chatId,
        ...
        (canCreateMore ? `💡 يمكنك إنشاء حساب آخر الآن!` : `⚠️ وصلت للحد الأقصى (حسابان). انتظر انتهاء المراجعة.`),
        { parse_mode: "Markdown", ...MAIN_MENU }
      );
    ...
    if (text === "📝 سجل الرصيد") {
      ...
      if (!tasks.length) { bot.sendMessage(chatId, `📝 *لا يوجد سجل رصيد بعد*\n\nاكمل مهام Gmail للحصول على رصيد!`, { parse_mode: "Markdown" }); return; }
      let txt = `📝 *سجل الرصيد*\n\n`; let total = 0;
      for (const t of tasks) {
    ...
    if (text === "💳 سحب") {
      ...
      const NETWORK_MENU = { reply_markup: { keyboard: [["🪙 Litecoin (LTC) | 0% +0.02$ | min: 0.20$"], ["💎 Tether (USDT-BEP-20) | 0% +0.03$ | min: 0.20$"], ["🔙 رجوع"]], resize_keyboard: true } };
      bot.sendMessage(chatId, `💸 *اختر شبكة السحب*\n\n💰 رصيدك: *$${fmt(user.balance)} USDT*\n\n⚠️ الحد الأدنى للسحب: *$0.20 USDT*\n\nاختر الشبكة المناسبة:`, { parse_mode: "Markdown", ...NETWORK_MENU }); return;
    }
    ...
    if (user.state === "awaiting_withdraw_network") {
      ...
      else if (text.includes("Tether") || text.includes("USDT-BEP-20")) { network = "USDT-BEP20"; fee = 0.03; feeAmount = 0.03; }
      else if (text === "🔙 رجوع") { user.state = null; await user.save(); bot.sendMessage(chatId, `👋 *القائمة الرئيسية*`, { parse_mode: "Markdown", ...MAIN_MENU }); return; }
      else { bot.sendMessage(chatId, "❌ اختر شبكة صحيحة."); return; }
      const totalNeeded = 0.20 + feeAmount;
      if (user.balance < totalNeeded) {
        bot.sendMessage(chatId, `❌ *رصيدك غير كافٍ*\n\n💰 رصيدك: *$${fmt(user.balance)} USDT*\n📉 تحتاج على الأقل: *$${fmt(totalNeeded)} USDT*\n(الحد الأدنى $0.20 + رسوم الشبكة $${fee})\n\nاكمل مهام Gmail لزيادة رصيدك!`, { parse_mode: "Markdown", ...MAIN_MENU });
      ...
      bot.sendMessage(chatId, `💸 *طلب سحب عبر ${network}*\n\n💰 رصيدك: *$${fmt(user.balance)} USDT*\n💸 رسوم الشبكة: *$${fee} USDT*\n📉 الحد الأدنى: *$0.20 USDT*\n\nأدخل المبلغ اللي بغيتي تسحبو:`, { parse_mode: "Markdown" }); return;
    }

    if (user.state === "awaiting_withdraw_amount_network") {
      const amount = parseFloat(text.trim());
      const { network, fee, feeAmount } = user.stateMeta || {};
      if (isNaN(amount) || amount < 0.20) { bot.sendMessage(chatId, `❌ الحد الأدنى $0.20. أدخل مبلغاً صحيحاً:`); return; }
      const totalDeduction = amount + feeAmount;
      if (totalDeduction > user.balance) { bot.sendMessage(chatId, `❌ رصيدك غير كافٍ. تحتاج $${fmt(totalDeduction)} (المبلغ + الرسوم).`, { parse_mode: "Markdown" }); return; }
      user.state = "awaiting_withdraw_address_network"; user.stateMeta = { ...user.stateMeta, amount }; await user.save();
      bot.sendMessage(chatId, `📮 أدخل عنوان محفظتك *(${network})*:\n\n⚠️ تأكد من العنوان، أي خطأ سيؤدي لضياع الأموال!`, { parse_mode: "Markdown" }); return;
    }

    if (user.state === "awaiting_withdraw_address_network") {
      const address = text.trim();
      const { network, fee, feeAmount, amount } = user.stateMeta || {};
      if (!address || address.length < 10) { bot.sendMessage(chatId, "❌ عنوان غير صحيح. حاول مرة أخرى:"); return; }
      const totalDeduction = amount + feeAmount;
      if (totalDeduction > user.balance) { user.state = null; user.stateMeta = null; await user.save(); bot.sendMessage(chatId, `❌ رصيدك غير كافٍ.`, { parse_mode: "Markdown", ...MAIN_MENU }); return; }
      ...
      bot.sendMessage(chatId, `✅ *تم إرسال طلب السحب!*\n\n🌐 الشبكة: *${network}*\n💵 المبلغ: *$${fmt(amount)} USDT*\n💸 رسوم الشبكة: *$${fmt(feeAmount)} USDT*\n📉 الإجمالي المخصوم: *$${fmt(totalDeduction)} USDT*\n📮 العنوان: \`${address}\`\n\n⏳ سيتم المعالجة خلال 24 ساعة.`, { parse_mode: "Markdown", ...MAIN_MENU });
      bot.sendMessage(ADMIN_ID, `💸 *طلب سحب جديد*\n\n👤 ${user.firstName} (\`${user.telegramId}\`)\n🌐 الشبكة: *${network}*\n💵 المبلغ: $${fmt(amount)} USDT\n💸 الرسوم: $${fmt(feeAmount)} USDT\n📮 \`${address}\`\n\nللموافقة: /approvew ${withdrawal._id}\nللرفض: /rejectw ${withdrawal._id}`, { parse_mode: "Markdown" }).catch(() => {});
      return;
    }

    if (text === "🔙 رجوع") { bot.sendMessage(chatId, `👋 *القائمة الرئيسية*`, { parse_mode: "Markdown", ...MAIN_MENU }); return; }

    if (text === "👥 الإحالات الخاصة بي") {
      const botInfo = await bot.getMe();
      const link = `https://t.me/${botInfo.username}?start=${user.referralCode}`;
      bot.sendMessage(chatId, `👥 *الإحالات الخاصة بك*\n\n🔗 رابطك:\n\`${link}\`\n\n👤 إجمالي الإحالات: *${user.referralCount}*`, { parse_mode: "Markdown", ...MAIN_MENU }); return;
    }

    if (text === "⚙️ الإعدادات") {
      bot.sendMessage(chatId, `⚙️ *الإعدادات*\n\n👤 الاسم: ${user.firstName}\n🆔 ID: \`${user.telegramId}\`\n💰 الرصيد: $${fmt(user.balance)}\n\nللسحب أرسل: /withdraw`, { parse_mode: "Markdown", ...MAIN_MENU }); return;
    }

    if (text === "💬 مساعدة") {
      bot.sendMessage(chatId,
        `💬 *المساعدة*\n\n❓ *كيف أنشئ حساب Gmail؟*\n1. افتح accounts.google.com\n2. اضغط "إنشاء حساب"\n3. أدخل البيانات المعطاة بالضبط\n4. أضف إيميل الاستعادة: *ryal2422@gmail.com*\n5. أكمل التحقق برقم الهاتف\n6. ارجع للبوت واضغط "تم"\n\n⚠️ *تنبيهات:*\n• استخدم البيانات المحددة فقط\n• إيميل الاستعادة يجب أن يكون: ryal2422@gmail.com\n• الحسابات المكررة ستُرفض\n• لا تغير كلمة المرور\n\nللتواصل مع الدعم: @𝑪𝒍𝒐𝒖𝒅𝑿`,
        { parse_mode: "Markdown", ...MAIN_MENU }
      ); return;
    }
...
bot.onText(/\/generate (\d+)/, async (msg, match) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      await Account.create({ firstName, lastName, email, password, recoveryEmail: RECOVERY_EMAIL });
      added++; generatedAccounts.push({ firstName, lastName, email, password });
    } catch (err) {
      if (err.code === 11000) {
        const newEmail = generateRandomEmail(firstName, lastName);
        try { await Account.create({ firstName, lastName, email: newEmail, password, recoveryEmail: RECOVERY_EMAIL }); added++; generatedAccounts.push({ firstName, lastName, email: newEmail, password }); }
        catch { failed++; }
      } else { failed++; }
    }
  }
  ...
  if (generatedAccounts.length > 0) {
    ...
    generatedAccounts.slice(-5).forEach((acc, i) => { summary += `${i+1}. \`${acc.email}\` | ${acc.firstName} ${acc.lastName}\n`; });
  }
  bot.sendMessage(msg.chat.id, summary, { parse_mode: "Markdown" });
  if (generatedAccounts.length > 0) {
    const csvContent = ["First Name,Last Name,Email,Password,Recovery Email", ...generatedAccounts.map(a => `${a.firstName},${a.lastName},${a.email},${a.password},${RECOVERY_EMAIL}`)].join("\n");
...
bot.onText(/\/addaccount (\S+) (\S+) (\S+) (\S+)/, async (msg, match) => {
  ...
  } catch { bot.sendMessage(msg.chat.id, "❌ الإيميل موجود بالفعل."); }
});
...
bot.onText(/\/addaccounts (.+)/s, async (msg, match) => {
  ...
  for (const line of lines) {
    ...
    try { await Account.create({ firstName, lastName, email, password, recoveryEmail: RECOVERY_EMAIL }); added++; }
    catch { failed++; }
  }
  const total = await Account.countDocuments({ assigned: false });
...
bot.onText(/\/approve (\w+)/, async (msg, match) => {
  ...
  if (user) {
    user.balance += task.amount; await user.save();
    bot.sendMessage(task.userId, `✅ *تمت الموافقة على حسابك!*\n\n📧 \`${task.accountEmail}\`\n💵 +$${task.amount} USDT\n💰 رصيدك: *$${fmt(user.balance)} USDT*`, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
  }
  bot.sendMessage(msg.chat.id, `✅ تمت الموافقة وإضافة $${task.amount} USDT.`);
});
...
bot.onText(/\/reject (\w+)/, async (msg, match) => {
  ...
  if (task.accountId) await Account.findByIdAndUpdate(task.accountId, { assigned: false, assignedTo: null, assignedAt: null });
  const user = await User.findOne({ telegramId: task.userId });
  if (user) bot.sendMessage(task.userId, `❌ *تم رفض الحساب*\n\n📧 \`${task.accountEmail}\`\nيمكنك المحاولة مرة أخرى.`, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
...
bot.onText(/\/approvew (\w+)/, async (msg, match) => {
  ...
  if (user) bot.sendMessage(withdrawal.userId, `✅ *تمت الموافقة على طلب السحب!*\n\n🌐 ${withdrawal.network}\n💵 $${fmt(withdrawal.amount)} USDT\n📮 \`${withdrawal.address}\``, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
...
bot.onText(/\/rejectw (\w+)/, async (msg, match) => {
  ...
  if (user) {
    ...
    bot.sendMessage(withdrawal.userId, `❌ *تم رفض طلب السحب*\n\n🌐 ${withdrawal.network}\n💵 تم إعادة $${fmt(withdrawal.totalDeduction)} USDT\n💰 رصيدك: *$${fmt(user.balance)} USDT*`, { parse_mode: "Markdown", ...MAIN_MENU }).catch(() => {});
  }
  bot.sendMessage(msg.chat.id, `❌ تم رفض السحب وإرجاع الرصيد.`);
});
...
bot.onText(/\/pending/, async (msg) => {
  ...
  const [users, accounts] = await Promise.all([User.find({ telegramId: { $in: userIds } }, "firstName telegramId"), Account.find({ _id: { $in: accountIds } }, "password recoveryEmail")]);
  ...
  for (const t of tasks) {
    ...
    const remainingMs = (t.submittedAt.getTime() + 72 * 60 * 60 * 1000) - Date.now();
    const timeLeft = Math.ceil(remainingMs / (60 * 60 * 1000));
    const timeLeftMin = Math.ceil(remainingMs / (60 * 1000)) % 60;
    const timeDisplay = timeLeft > 0
      ? `${timeLeft} ساعة و ${timeLeftMin} دقيقة`
      : "انتهى الوقت";
...
// ─── /checking - حسابات قيد التحقق ──────────────────────────────────────────
bot.onText(/\/checking/, async (msg) => {
  if (msg.from.id !== ADMIN_ID) return;

  const stuckUsers = await User.find({ state: "awaiting_confirmation" });
  if (!stuckUsers.length) {
    bot.sendMessage(msg.chat.id, "✅ لا يوجد أحد في حالة التحقق حالياً.");
    return;
  }

  let text = `🔍 *الحسابات قيد التحقق (${stuckUsers.length})*\n\n`;
  for (const user of stuckUsers) {
    const accountId = user.stateMeta?.accountId;
    ...
    const timeDisplay = sinceHours >= 1
      ? `${sinceHours} ساعة و ${sinceMinutes % 60} دقيقة`
      : `${sinceMinutes} دقيقة`;
    ...
    if (account) {
      ...
      text += `🔗 \`${account.recoveryEmail || RECOVERY_EMAIL}\`\n`;
    } else {
      text += `📧 حساب غير متاح\n`;
    }
...
bot.onText(/\/accounts/, async (msg) => {
  ...
  bot.sendMessage(msg.chat.id, `📦 *الحسابات*\n\n✅ متاح: *${available}*\n🔒 مُعيَّن: *${assigned}*\n📊 الإجمالي: *${available + assigned}*`, { parse_mode: "Markdown" });
});
...
bot.onText(/\/withdraw/, async (msg) => {
  ...
  user.state = "awaiting_withdraw_network"; user.stateMeta = null; await user.save();
  const NETWORK_MENU = { reply_markup: { keyboard: [["🪙 Litecoin (LTC) | 0% +0.02$ | min: 0.20$"], ["💎 Tether (USDT-BEP-20) | 0% +0.03$ | min: 0.20$"], ["🔙 رجوع"]], resize_keyboard: true } };
  bot.sendMessage(msg.chat.id, `💸 *اختر شبكة السحب*\n\n💰 رصيدك: *$${fmt(user.balance)} USDT*\n\n⚠️ الحد الأدنى للسحب: *$0.20 USDT*\n\nاختر الشبكة المناسبة:`, { parse_mode: "Markdown", ...NETWORK_MENU });
});
